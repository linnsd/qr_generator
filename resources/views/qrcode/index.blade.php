                ...
                <div class="card w-100">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive" style="font-size:14px;">
                            <a href="{{ url('/qr_create') }}" class="btn btn-primary btn-sm form-group"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <input type="text" name="item_name" id="item_name"
                                            class="form-control form-group" placeholder="Search Item..."
                                            value="{{ old('item_name', $item_name) }}">
                                    </div>
                                    <div class="col-md-2">
                                    ...
                                    <div class="col-md-8" align="right">
                                        <a href="{{ route('qr.create') }}" class="btn btn-sm btn-success"><i
                                ...
                                <tbody>
                                    @foreach ($qr_list as $key => $qr)
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-header">
                                                        ...
                                                        <button type="button" class="btn btn-sm" data-bs-dismiss="modal"><i
                                                                class="fa fa-window-close"></i></button>
                                                    </div>

                                                    <!-- Modal body -->
                                                    <div class="modal-body">
                                                        <form action="{{ route('update_remark', $qr->id) }}" method="POST"
                                                            accept-charset="utf-8" class="form-horizontal">
                                                            @csrf
                                                            <!-- <p class="text text-muted">{{ $qr->qr_link }}</p> -->
                                                            <input type="text" name=""
                                                                value="{{ $qr->qr_link }}" class="form-control form-group"
                                                                readonly>
                                                            <input type="text" name="remark" id="remark"
                                                                class="form-control form-group" value="{{ $qr->remark }}"
                                                                placeholder="Item Name">
                                                            <select name="category" class="form-control form-group">
                                                                <option value="">Select</option>
                                                                @foreach ($categories as $cat)
                                                                    <option value="{{ $cat->id }}"
                                                                        {{ $qr->category_id == $cat->id ? 'selected' : '' }}>
                                                                        {{ $cat->name }}</option>
                                                                @endforeach
                                                            </select>
                                        ...
                                        <tr>
                                            <td>{{ ++$i }}</td>
                                            <td>
                                                <img src="{{ asset($qr->path . $qr->photo) }}"
                                            ...
                                            <td>
                                                ...
                                                <form action="{{ route('qr.download') }}" method="post">
                                                    <div class="d-flex">
                                                        ...
                                                        @method('Post')
                                                        @can('qr-edit')
                                                            <button id="alert_modal{{ $qr->id }}" type="button"
                                                                class="btn btn-sm btn-primary mr-1"
                                                                data-remark="{{ $qr->remark }}"
                                                                data-id="{{ $qr->id }}">
                                                                <i class="fa fa-fw fa-edit"></i>
                                                            </button>
                                                        @endcan

                                                        <a href="{{ url('print_all_qr?photo_path=' . $qr->photo) }}"
                                                            target="_blank" class="btn btn-sm btn-secondary mr-1"><i
                                                                class="fas fa-fw fa-print"></i></a>

                                                        @can('qr-download')
                                                            <input type="hidden" name="qr_path"
                                                                value="{{ $qr->path }}">
                                                            <input type="hidden" name="qr_photo"
                                                                value="{{ $qr->photo }}">
